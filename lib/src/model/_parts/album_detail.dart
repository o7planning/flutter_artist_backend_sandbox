part of '../model.dart';

@JsonSerializable()
class AlbumDetail {
  @JsonKey(name: 'id')
  int id;

  @JsonKey(name: 'code')
  String code;

  @JsonKey(name: 'name')
  String name;

  @JsonKey(name: 'seqNum')
  int seqNum;

  @JsonKey(name: 'published')
  bool published;

  @JsonKey(name: 'locked')
  bool locked;

  @JsonKey(name: 'imagePath')
  String? imagePath;

  @JsonKey(name: 'nameInEnglish')
  String? nameInEnglish;

  @JsonKey(name: 'publishedSongCount')
  int publishedSongCount;

  @JsonKey(name: 'totalSongCount')
  int totalSongCount;

  @JsonKey(name: 'description')
  String? description;

  @JsonKey(name: 'note')
  String? note;

  AlbumDetail(
    this.id,
    this.code,
    this.name,
    this.seqNum,
    this.published,
    this.locked,
    this.publishedSongCount,
    this.totalSongCount,
  );

  AlbumDetail.named({
    required this.id,
    required this.code,
    required this.name,
    required this.seqNum,
    required this.published,
    required this.locked,
    required this.publishedSongCount,
    required this.totalSongCount,
    this.imagePath,
    this.description,
    this.note,
  });

  factory AlbumDetail.fromEntity(AlbumData entity) {
    return AlbumDetail.named(
      id: entity.id,
      code: entity.code,
      name: entity.name,
      seqNum: entity.seqNum,
      published: entity.published,
      locked: entity.locked,
      publishedSongCount: entity.publishedSongCount,
      totalSongCount: entity.totalSongCount,
      imagePath: entity.imagePath,
      description: entity.description,
      note: entity.note,
    );
  }

  AlbumInfo toAlbumInfo() {
    return AlbumInfo.named(
      id: id,
      code: code,
      name: name,
      seqNum: seqNum,
      published: published,
      locked: locked,
      imagePath: imagePath,
    );
  }

  factory AlbumDetail.fromJson(Map<String, dynamic> json) =>
      _$AlbumDetailFromJson(json);

  // IMPORTANT:
  Map<String, dynamic> toJson() => _$AlbumDetailToJson(this);
}
